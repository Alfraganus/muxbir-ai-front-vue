...
const AI_PROVIDERS = [
  { id: 'gemini',     label: 'Google Gemini',     note: '2.5 Flash Lite / Flash / Pro / 3.1 Pro' },
]
const AI_MODELS_BY_PROVIDER = {
  gemini: [
    { id: 'gemini-2.5-flash-lite',  label: 'gemini-2.5-flash-lite',  note: 'eng arzon, tez' },
    { id: 'gemini-2.5-flash',       label: 'gemini-2.5-flash',       note: 'arzon, tez (tavsiya)' },
    { id: 'gemini-2.5-pro',         label: 'gemini-2.5-pro',         note: 'kuchli' },
    { id: 'gemini-3.1-pro-preview', label: 'gemini-3.1-pro-preview', note: 'eng kuchli' },
  ],
}
...
async function openAutoSettings(channel, opts = {}) {
  ...
  comparePairsList.value = Array.isArray(channel.compare_providers) && channel.compare_providers.length
    ? channel.compare_providers.map(p => ({ ...p }))
    : [{ provider: 'gemini', model: 'gemini-2.5-pro' }, { provider: 'anthropic', model: 'claude-opus-4-8' }]
...
async function saveAutoSettings() {
  ...
  try {
    ...
    updated = await channelsApi.updateAutoSettings(company.value.id, updated.id, {
      ...
      compare_mode_enabled: compareEnabled.value,
      compare_providers: compareEnabled.value ? comparePairsList.value : [],
...
// ── AI Solishtirish rejimi (kanal sozlamalarida) ──────────────
const compareEnabled = ref(false)
const comparePairsList = ref([
  { provider: 'gemini', model: 'gemini-2.5-pro' },
  { provider: 'anthropic', model: 'claude-opus-4-8' },
])

function onComparePairChange(idx) {
  const provider = comparePairsList.value[idx].provider
  const list = AI_MODELS_BY_PROVIDER[provider] || []
  if (list.length) comparePairsList.value[idx].model = list[0].id
}
