  ...
  <div class="magic-wrap">
    <div class="magic-card">
      ...
      <div v-else-if="state === 'success'" class="magic-state">
        ...
        <span style="font-weight:600;">Muvaffaqiyatli!</span>
        <span style="font-size:12px;color:var(--muted);">Yo'naltirilmoqda…</span>
      </div>
      <div v-else class="magic-state">
        <div class="magic-icon danger"><AppIcon name="Close" :size="22"/></div>
        <span style="font-weight:600;">Havola yaroqsiz</span>
        <span style="font-size:12px;color:var(--muted);">{{ errorMsg }}</span>
        <AppButton variant="secondary" size="md" @click="$router.push('/signin')">Qaytish</AppButton>
      </div>
    </div>
  </div>
</template>
...
onMounted(async () => {
  ...
  if (!token) {
    state.value = 'error'
    errorMsg.value = 'Token topilmadi'
    return
  }
  try {
    ...
    state.value = 'error'
    errorMsg.value = e?.response?.data?.message || 'Havola muddati o\'tgan yoki ishlatilgan'
  }
})
</script>
